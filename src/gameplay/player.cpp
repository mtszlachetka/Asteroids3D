...
namespace se
{
	player::player(
		...
		const obb &t_box) : transformable(t_position, t_orientation),
	...
	void player::update_mouse_offset(double x_offset, double y_offset)
	{
		...
		x_rotation = std::abs(glm::dot(glm::toMat3(x_rotation * m_orientation)[2], v3(0.f, 1.f, 0.f))) <= 0.9 ? x_rotation : qu(1.f, 0.f, 0.f, 0.f);
	...
	void player::collide_with(collidable *cl, collision_info *info)
	{
		...
		if (a_ptr != nullptr)
		{
			...
			float player_momentum = 4.f * (m_is_moving ? (m_boost_active ? m_boost_speed : m_base_speed) : 0.f);
