...
namespace se
{
	void gameplay_engine::init()
	{
		...
		m_station_ptr = std::make_unique<se::station>(glm::vec3(0.f), m_station_mesh, m_station_textures, m_explosion_program, m_station_sphere, m_station_obb, 100);
		m_player_ptr = std::make_unique<se::player>(glm::vec3(0.f, 0.f, 10.f), glm::quat(1, 0, 0, 0), m_player_mesh, m_player_textures, m_explosion_program, m_player_sphere, m_player_obb);
	}
	...
	void gameplay_engine::tick()
	{
		...
		m_asteroid_ptrs.remove_if([](std::unique_ptr<asteroid> &a) -> bool
								  { return a->get_should_destruct(); });
		m_missile_ptrs.remove_if([](std::unique_ptr<missile> &m) -> bool
								 { return m->get_should_destruct(); });
		...
		// spawn asteroids
		if (game_clock::get_instance().get_current_frame_time() - m_last_spawn_time > 2.f)
		{ // every 2 seconds
	...
	void gameplay_engine::spawn_missile()
	{
		...
		if (game_clock::get_instance().get_current_frame_time() - m_last_shot_time < m_shooting_cooldown)
			return;
		using v3 = glm::vec3;
	...
	void gameplay_engine::clear()
	{ // this is needed to avoid a segfault after exiting main loop
